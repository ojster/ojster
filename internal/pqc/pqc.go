...
func Seal(args []string) {
	...
	if fs.NArg() != 1 {
		log.Printf("seal requires exactly one positional argument: KEY")
		exitFunc(1)
	}
	keyName := fs.Arg(0)
	...
	if err != nil {
		log.Printf("failed to read public key file %s: %v", *pubPath, err)
		exitFunc(1)
	}

	pubBytes, err := base64.StdEncoding.DecodeString(strings.TrimSpace(string(pubBytesRaw)))
	if err != nil {
		log.Printf("invalid base64 public key in %s: %v", *pubPath, err)
		exitFunc(1)
	}

	ek, err := mlkem.NewEncapsulationKey768(pubBytes)
	if err != nil {
		log.Printf("invalid public key in %s: %v", *pubPath, err)
		exitFunc(1)
	}
	...
	if err != nil {
		log.Printf("failed to read plaintext: %v", err)
		exitFunc(1)
	}
	...
	if len(sharedKey) != mlkem.SharedKeySize {
		log.Printf("unexpected shared key size: %d", len(sharedKey))
		exitFunc(1)
	}
	...
	if err := env.UpdateEnvFile(*outPath, keyName, sealed); err != nil {
		log.Printf("failed to update env file %s: %v", *outPath, err)
		exitFunc(1)
	}
...
func Unseal(args []string) {
	...
	if err != nil {
		log.Printf("failed to read private key file %s: %v", *privPath, err)
		exitFunc(1)
	}
	privText := strings.TrimSpace(string(privFileBytes))
	privBytes, err := base64.StdEncoding.DecodeString(privText)
	if err != nil {
		log.Printf("invalid base64 private key in %s: %v", *privPath, err)
		exitFunc(1)
	}

	dk, err := mlkem.NewDecapsulationKey768(privBytes)
	if err != nil {
		log.Printf("invalid private key in %s: %v", *privPath, err)
		exitFunc(1)
	}
	...
	if err != nil {
		log.Printf("failed to read env file %s: %v", *inPath, err)
		exitFunc(1)
	}
	...
	for _, k := range keys {
		...
		if !strings.HasPrefix(stored, common.Prefix) {
			log.Printf("value for %s does not appear to be sealed (missing common.Prefix)", k)
			exitFunc(1)
		}
		payload := strings.TrimPrefix(stored, common.Prefix)
		parts := strings.SplitN(payload, common.Sep, 2)
		if len(parts) != 2 {
			log.Printf("sealed value for %s malformed", k)
			exitFunc(1)
		}
		mlkemB64 := parts[0]
		gcmB64 := parts[1]
		...
		if err != nil {
			log.Printf("invalid base64 mlkem ciphertext for %s: %v", k, err)
			exitFunc(1)
		}
		gcmBlob, err := base64.StdEncoding.DecodeString(gcmB64)
		if err != nil {
			log.Printf("invalid base64 gcm blob for %s: %v", k, err)
			exitFunc(1)
		}

		sharedKey, err := dk.Decapsulate(mlkemCiphertext)
		if err != nil {
			log.Printf("decapsulation failed for %s: %v", k, err)
			exitFunc(1)
		}
		if len(sharedKey) != mlkem.SharedKeySize {
			log.Printf("unexpected shared key size for %s: %d", k, len(sharedKey))
			exitFunc(1)
		}

		plaintext, err := decryptAESGCM(sharedKey, gcmBlob)
		if err != nil {
			log.Printf("decryption failed for %s: %v", k, err)
			exitFunc(1)
		}
